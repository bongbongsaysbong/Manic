execute if entity @e[type=minecraft:interaction,tag=manic.horror_ward.enabled,distance=..64] run return run kill @s
...
execute at @s if predicate nucleus:block/waterlogged run function manic:entity/technical/spectre/spawning/as_marker/randomise_water
