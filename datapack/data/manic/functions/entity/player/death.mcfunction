...
execute unless entity @s[gamemode=spectator] unless entity @a[gamemode=!creative,gamemode=!spectator,distance=0.1..16,scores={manic.sanity=..4999}] as @e[type=#manic:spectre,tag=manic.entity.despawns,tag=!nucleus.animating,distance=..16] at @s run function manic:entity/technical/spectre/despawn/despawn
